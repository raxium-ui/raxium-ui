...
<template>
  <div class="w-full flex flex-col gap-2">
    <div class="text-sm text-gray-cc">
      value: <span class="text-gray-ff">{{ value.length ? String(value[0]) : '(empty)' }}</span>
    </div>
    <div class="w-full flex items-center gap-4 max-w-[520px] rounded-md border border-gray-33 p-3">
      <DatePicker
        v-for="size in sizes"
        :key="size"
        v-model="value"
