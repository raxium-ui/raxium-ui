...
model: composer-1.5
description: >-
  当检测到 `.changeset/` 下有新的或占位 changeset `.md`（frontmatter 已有 bump 类型、正文为空/占位）时，
  根据当前工作区改动生成简明 changelog 并写回该文件。
  Use when a new changeset markdown exists under .changeset/, or the user asks to fill/write the changeset changelog.
---
...
2. changeset 正文仍是占位（如 `change log`、空行、`TODO`），或用户明确要求填写 changelog
